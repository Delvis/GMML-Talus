...
auto.alignment<-function(input.dir, output.dir, nlandmarks=c(32, 64)){
    ...
    tmp.dir <- getwd()
    setwd(input.dir)
    model.names<-gsub(pattern = ".off",replacement = "",x=dir(pattern = ".off"))
    setwd(output.dir)
    conn<-file(description = "model.names.txt",open = "wt")
    write(model.names,file = conn)
    close(conn)
    
    # Read grouping variable from last character in 3D models name
    group<-as.factor(substr(model.names,start = nchar(model.names),
    ...
    alignment.output<-align_shapes(Data_dir = input.dir,Output_dir = output.dir,
                                   Levels = nlandmarks,Ids =model.names,
                                   Names = model.names)
